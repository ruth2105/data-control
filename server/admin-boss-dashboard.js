...
function renderTeamPerformance() {
  getReports(function(reports) {
    ...
    const pdfCustomers = allPotentials.map(c => ({
      id: 'pdf_' + c.name.toLowerCase().replace(/\s+/g, '_'),
      hospitalName: c.name,
      salesPerson: normalizeSalesName(c.salesRep),
      teamId: Number(c.teamId) || null,
      customerPotential: c.status,
      source: 'PDF'
    }));

    const normalizeReport = function(r) {
      const report = {...r};
      report.salesPerson = normalizeSalesName(report.salesPerson || report.salesRep || report.username || 'Unknown');
      if (!report.teamId && typeof getTeamIdBySalesperson === 'function') {
        report.teamId = getTeamIdBySalesperson(report.salesPerson);
      }
      report.teamId = report.teamId != null && report.teamId !== '' ? Number(report.teamId) : null;
      return report;
    };
    
    const allData = [...reports.map(normalizeReport), ...pdfCustomers];
    ...
    const html = teams.map(teamId => {
      ...
      console.log('Local users for team performance:', localUsers); // Debug
      localUsers.forEach(u => {
        if (u.username && String(u.status || '').toLowerCase() === 'active') {
          allUsers[u.username] = u;
        }
      });
      
      const teamMembers = Object.values(allUsers).filter(u => u.role === 'sales' && Number(u.teamId) === teamId);
      console.log(`Team ${teamId} members:`, teamMembers); // Debug
